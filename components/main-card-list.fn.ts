...
export const useComputed = (root: any) =>
  reactive<IComputed>({
    mainData: computed(() => {
      return root.$store.getters[MainConstant.$Get.MainData]
    }),
  })

export const useStartTimeSave = (root: any, state: IState) => async () => {
  await root.$store.dispatch(CommuteConstant.$Call.CommutePost, {
    companyId: 1,
  ...
  root.$store.dispatch(SnackConstant.$Call.SnackStatus, {
